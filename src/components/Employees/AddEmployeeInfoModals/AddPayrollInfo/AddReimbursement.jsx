...
const AddReimbursement = ({ visible, setVisible }) => {
  ...
  const formik = useFormik({
    ...
    onSubmit: async (values) => {
      try {
        ...
        formik.resetForm()
        setVisible(false)
        toast.success('Reimbursement Info Added Successfully')
      } catch (err) {
        console.log(err)
        toast.error('Failed to add Reimbursement Info')
      }
    },
  })
  ...
  return (
    <CModal size="lg" alignment="center" visible={visible} onClose={() => setVisible(false)}>
      <CModalHeader onClose={() => setVisible(false)}>
        <h3>Add Reimbursement Allowances</h3>
      </CModalHeader>
